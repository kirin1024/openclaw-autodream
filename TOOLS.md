...
## 架构
```
群消息 → main → 关键词识别 → 匹配则转发小学 → 小学处理 → 返回main → main发送+统计 → 群
```
...
## 小学会话标识
- session_key: `agent:small-school:subagent:4d107250-6400-49ea-8d44-ef051e678dc7`
- 注意：此 session_key 固定不变，每次使用 sessions_send 发送到此 key

## Token 统计
- 由 main 统一附加到回复末尾
- 格式：`[📊 Tokens: xxx in / xxx out | 模型: xxx]`
- 统计来源：sessions_send 的返回结果中的 usage 字段
